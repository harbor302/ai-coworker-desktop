...
describe('SessionManager processQueue concurrency', () => {
  ...
  it('returns running status for newly started sessions immediately', async () => {
    const manager = new SessionManager(db, sendToRenderer);

    let resolvePrompt!: () => void;
    const promptWork = new Promise<void>((resolve) => {
      resolvePrompt = resolve;
    });
    (manager as unknown as { processPrompt: () => Promise<void> }).processPrompt = vi
      .fn()
      .mockReturnValue(promptWork);

    const session = await manager.startSession('Test', 'first prompt', '/tmp');
  ...
  it('restarts queued prompts that arrive while a stopped run is cleaning up', async () => {
    const manager = new SessionManager(db, sendToRenderer);
    ...
    await vi.waitFor(() => expect(processPromptSpy).toHaveBeenCalledTimes(1));
    ...
    resolveFirst();
    await vi.waitFor(() => expect(processPromptSpy).toHaveBeenCalledTimes(2));
    ...
    expect(statusUpdates.filter((status) => status === 'running')).toHaveLength(2);
  });
});
