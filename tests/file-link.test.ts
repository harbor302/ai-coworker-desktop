...
describe('splitTextByFileMentions', () => {
  ...
  it('detects absolute paths', () => {
    const input = '路径 /Users/demo/test/报告.docx 已生成';
    const parts = splitTextByFileMentions(input);
    expect(parts).toEqual([
      { type: 'text', value: '路径 ' },
      { type: 'file', value: '/Users/demo/test/报告.docx' },
      { type: 'text', value: ' 已生成' },
    ]);
  });

  it('detects absolute paths with spaces', () => {
    const input = '文档已保存为：/Users/demo/Library/Application Support/coworker/default_working_dir/word-document/示例文档.docx';
    const parts = splitTextByFileMentions(input);
    expect(parts).toEqual([
      { type: 'text', value: '文档已保存为：' },
      { type: 'file', value: '/Users/demo/Library/Application Support/coworker/default_working_dir/word-document/示例文档.docx' },
    ]);
  });
