...
describe('runDiagnostics TLS step', () => {
  ...
  beforeEach(() => {
    ...
    mocks.probeWithPiSdk.mockResolvedValue({ ok: true, latencyMs: 10 });
  ...
  it('model step uses probeWithPiSdk', async () => {
    mocks.probeWithPiSdk.mockResolvedValue({ ok: true, latencyMs: 15 });
    ...
    expect(result.overallOk).toBe(true);
    expect(mocks.probeWithPiSdk).toHaveBeenCalledWith(
  ...
  it('model step reports failure from probeWithPiSdk', async () => {
    mocks.probeWithPiSdk.mockResolvedValue({
      ok: false,
