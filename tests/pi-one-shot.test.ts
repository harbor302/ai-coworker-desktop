...
vi.mock('electron-store', () => {
  class MockStore<T extends Record<string, unknown>> {
    ...
    public path = '/tmp/mock-pi-one-shot-config.json';
...
import { probeWithPiSdk } from '../src/main/agent/pi-one-shot';
...
describe('probeWithPiSdk', () => {
  ...
  it('does not fall back to saved api key when the draft explicitly clears it', async () => {
    const result = await probeWithPiSdk(
      {
        provider: 'openai',
        apiKey: '',
        model: 'gpt-5.4',
