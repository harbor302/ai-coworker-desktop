...
describe('getArtifactSteps', () => {
  ...
  it('formats label from full path', () => {
    expect(getArtifactLabel('/Users/demo/tmp/simple.md')).toBe('simple.md');
  });

  it('uses basename when path exists even if name provided', () => {
    expect(getArtifactLabel('/Users/demo/tmp/simple.md', '自定义名称')).toBe('simple.md');
  });
  ...
  it('prefers basename over translated name', () => {
    expect(getArtifactLabel('/Users/demo/tmp/simple.pptx', '简单PPT演示文稿')).toBe('simple.pptx');
  });
});
