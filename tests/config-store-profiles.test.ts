...
describe('ConfigStore provider profiles', () => {
  ...
  it('preserves fetched-model mode on provider profiles', () => {
    mocks.seed = {
      provider: 'custom',
      customProtocol: 'openai',
    ...
    expect(config.profiles['custom:openai']?.model).toBe('deepseek-chat');
    expect(config.profiles['custom:openai']?.useCustomModel).toBe(false);
  });
