...
describe('AgentRunner cowoker SDK integration', () => {
  ...
  it('instructs the model to read connector MCP usage skills before MCP calls', () => {
    expect(agentRunnerContent).toContain('function buildMcpUsageInstructionsPrompt');
    expect(agentRunnerContent).toContain('<mcp_usage_instructions>');
    expect(agentRunnerContent).toContain('read its SKILL.md with the Read tool');
    expect(agentRunnerContent).toContain('getMcpToolUsagePrefix');
    expect(agentRunnerContent).toContain('Before using ${definition.displayName} MCP tools');
  });
  ...
  it('passes frontend image blocks to the pi agent prompt as image attachments', () => {
    expect(agentRunnerContent).toContain('function extractPromptImages');
    expect(agentRunnerContent).toContain('mimeType: block.source.media_type');
    expect(agentRunnerContent).toContain('const promptImages = extractPromptImages');
    expect(agentRunnerContent).toContain(
