...
describe('ConfigStore config sets', () => {
  ...
  it('allows deleting the default set when another set remains', () => {
    const store = new ConfigStore();
    ...
    const afterDefaultDelete = store.deleteSet({ id: 'default' });
    expect(afterDefaultDelete.activeConfigSetId).toBe(customSet!.id);
    expect(afterDefaultDelete.configSets).toHaveLength(1);
    expect(afterDefaultDelete.configSets.some((set) => set.id === 'default')).toBe(false);
  });

  it('falls back to the first remaining set after deleting the active set', () => {
    const store = new ConfigStore();
    const created = store.createSet({ name: 'My Set', mode: 'clone' });
    const customSet = created.configSets.find((set) => !set.isSystem);
    expect(customSet).toBeTruthy();
  ...
  it('allows renaming default set but still blocks deleting the last remaining set', () => {
    const store = new ConfigStore();
