...
describe('api config state helpers', () => {
  ...
  it('keeps an API-fetched model in fetched-model mode after save', () => {
    const config = {
      provider: 'custom',
      customProtocol: 'openai',
      activeProfileKey: 'custom:openai',
    ...
    const snapshot = buildApiConfigSnapshot(config, FALLBACK_PROVIDER_PRESETS);
    expect(snapshot.profiles['custom:openai'].useCustomModel).toBe(false);
    expect(snapshot.profiles['custom:openai'].model).toBe('deepseek-chat');
    expect(snapshot.profiles['custom:openai'].customModel).toBe('');
  });

  it('does not ship hardcoded model preset lists and keeps custom guidance', () => {
    expect(FALLBACK_PROVIDER_PRESETS.openrouter.models).toEqual([]);
    expect(FALLBACK_PROVIDER_PRESETS.openai.models).toEqual([]);
    expect(FALLBACK_PROVIDER_PRESETS.anthropic.models).toEqual([]);
    expect(FALLBACK_PROVIDER_PRESETS.gemini.models).toEqual([]);
