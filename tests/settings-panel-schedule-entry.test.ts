...
// Schedule was moved out of SettingsPanel into a first-level left-sidebar page.
const appPath = path.resolve(process.cwd(), 'src/renderer/App.tsx');
const sidebarPath = path.resolve(process.cwd(), 'src/renderer/components/Sidebar.tsx');
const schedulePagePath = path.resolve(process.cwd(), 'src/renderer/components/SchedulePage.tsx');
...
const settingsPanelContent = [
  ...
  readFileSync(schedulePagePath, 'utf8'),
  readFileSync(settingsPanelPath, 'utf8'),
  ...readdirSync(settingsDir).map((f) => readFileSync(path.join(settingsDir, f), 'utf8')),
].join('\n');

describe('Schedule page entry', () => {
  it('renders schedule as a first-level page from the left sidebar', () => {
    expect(settingsPanelContent).toContain('import { SchedulePage }');
    expect(settingsPanelContent).toContain('showSchedule ? (');
    expect(settingsPanelContent).toContain(
      '<SchedulePage onClose={() => setShowSchedule(false)} />'
    );
    expect(settingsPanelContent).toContain('const handleOpenSchedule = () => {');
    expect(settingsPanelContent).toContain("title={t('settings.schedule')}");
  });

  it('removes schedule from SettingsPanel tabs', () => {
    expect(readFileSync(settingsPanelPath, 'utf8')).not.toContain("id: 'schedule' as TabId");
  });
  ...
  it('avoids resetting schedule time when editing without changing runAt', () => {
    ...
    expect(settingsPanelContent).toContain(
      'toLocalDateTimeInput(runAtValue) !== originalRunAtInput'
    );
  });
