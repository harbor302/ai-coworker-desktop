...
describe('resolveMessageEndPayload', () => {
  ...
  it('does not mark unknown message_end errors as auto-retryable', () => {
    const result = resolveMessageEndPayload({
      message: {
        role: 'assistant',
        content: [],
    ...
    expect(result.shouldEmitMessage).toBe(false);
    expect(result.shouldAutoRetry).toBe(false);
    expect(result.errorText).toBe('An unknown error occurred');
  });

  it('marks transient upstream message_end errors as auto-retryable', () => {
    const result = resolveMessageEndPayload({
      message: {
        role: 'assistant',
        content: [],
    ...
    expect(result.errorText).toContain('上游服务异常');
  });
...
describe('shouldAutoRetryError', () => {
  it('only marks transient errors as auto-retryable', () => {
    expect(shouldAutoRetryError('HTTP 502: Bad Gateway')).toBe(true);
    expect(shouldAutoRetryError('connection error: ECONNRESET')).toBe(true);
    expect(shouldAutoRetryError('An unknown error occurred')).toBe(false);
    expect(shouldAutoRetryError('HTTP 400: bad request')).toBe(false);
    expect(shouldAutoRetryError('Cannot continue from message role: assistant')).toBe(false);
  });
});
