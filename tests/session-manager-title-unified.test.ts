...
vi.mock('../src/main/agent/pi-one-shot', () => ({
  generateTitleWithPiSdk: vi.fn(async () => 'Unified Title'),
}));

import { configStore } from '../src/main/config/config-store';
import { SessionManager } from '../src/main/session/session-manager';
import { generateTitleWithPiSdk } from '../src/main/agent/pi-one-shot';

const mockedGenerateTitleWithPiSdk = vi.mocked(generateTitleWithPiSdk);
...
describe('SessionManager unified title generation', () => {
  ...
  beforeEach(() => {
    ...
    mockedGenerateTitleWithPiSdk.mockClear();
  });
  ...
  it('routes title generation through Pi SDK in unified mode', async () => {
    ...
    expect(mockedGenerateTitleWithPiSdk).toHaveBeenCalledTimes(1);
    expect(mockedGenerateTitleWithPiSdk).toHaveBeenCalledWith(
  ...
  it('routes gemini title generation through Pi SDK even when unified mode flag is disabled', async () => {
