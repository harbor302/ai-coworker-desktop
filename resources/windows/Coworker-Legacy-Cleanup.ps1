...
function Write-Step {
  ...
  Write-Host "[Coworker Cleanup] $Message"
}
...
function Get-CoworkerRegistryEntries {
  $registryGlobs = @(
  ...
  foreach ($glob in $registryGlobs) {
    ...
    $items = Get-ItemProperty -Path $glob -ErrorAction SilentlyContinue | Where-Object {
      $_.DisplayName -like "Coworker*" -or $_.Publisher -eq "Independent Developer"
    }
...
function Stop-CoworkerProcesses {
  $processes = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object {
    $_.Name -ieq "Coworker.exe" -or $_.ExecutablePath -like "*\Coworker.exe"
  })

  if ($processes.Count -eq 0) {
    Write-Step "No running Coworker processes found."
    return
  }
...
$registryEntries = @(Get-CoworkerRegistryEntries)
...
Add-UniquePath -List $installPaths -PathValue (Join-Path $env:LOCALAPPDATA "Programs\Coworker")

$appDataPaths = [System.Collections.Generic.List[string]]::new()
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "Coworker")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:APPDATA "coworker")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "Coworker")
Add-UniquePath -List $appDataPaths -PathValue (Join-Path $env:LOCALAPPDATA "coworker")

Write-Host ""
Write-Step "This tool removes broken Coworker Windows install leftovers."
...
if ($failures.Count -eq 0) {
  Write-Step "Cleanup finished. You can rerun the Coworker installer now."
