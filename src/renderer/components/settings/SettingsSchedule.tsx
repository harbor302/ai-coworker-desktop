...
import {
  describeScheduleTextFromConfig,
  parseNaturalScheduleText,
} from '../../../shared/schedule/natural-language-parser';
...
const NATURAL_SCHEDULE_SUGGESTIONS = [
  '明天早上 9 点',
  '每天早上 9 点',
  '每周一上午 10 点',
  '工作日 8:30',
...
export function SettingsSchedule({ isActive }: { isActive: boolean }) {
  ...
  const [scheduleText, setScheduleText] = useState('明天早上 9 点');
  ...
  const parsedNaturalSchedule = useMemo(
    () => parseNaturalScheduleText(scheduleText),
    [scheduleText]
  );
  ...
  async function submitTask() {
    ...
    const useNaturalSchedule = !useAdvancedSchedule;
    const effectiveScheduleMode: ScheduleFormMode = useNaturalSchedule
      ? parsedNaturalSchedule.ok
        ? parsedNaturalSchedule.mode === 'interval'
          ? 'legacy-interval'
          : parsedNaturalSchedule.mode
        : scheduleMode
      : scheduleMode;
    const effectiveScheduleConfig = useNaturalSchedule
      ? parsedNaturalSchedule.ok
        ? (parsedNaturalSchedule.scheduleConfig as ScheduleConfig | null)
        : null
      : scheduleConfig;
    const effectiveRepeatEvery = useNaturalSchedule
      ? parsedNaturalSchedule.ok
        ? parsedNaturalSchedule.repeatEvery
        : null
      : scheduleMode === 'legacy-interval'
        ? repeatEvery
        : null;
    const effectiveRepeatUnit = useNaturalSchedule
      ? parsedNaturalSchedule.ok
        ? (parsedNaturalSchedule.repeatUnit as ScheduleRepeatUnit | null)
        : null
      : scheduleMode === 'legacy-interval'
        ? repeatUnit
        : null;

    if (useNaturalSchedule && !parsedNaturalSchedule.ok) {
      setError({ text: parsedNaturalSchedule.reason });
      return;
    }
    if (
      effectiveScheduleMode === 'daily' &&
      (!effectiveScheduleConfig || effectiveScheduleConfig.times.length === 0)
    ) {
    ...
    if (effectiveScheduleMode === 'weekly') {
      if (
        ...
        effectiveScheduleConfig.kind !== 'weekly' ||
        effectiveScheduleConfig.times.length === 0
      ) {
        setError({ key: 'schedule.weeklyTimesRequired' });
        return;
      }
      if (effectiveScheduleConfig.weekdays.length === 0) {
        setError({ key: 'schedule.weekdayRequired' });
        return;
      }
    }
    const usesDateTimeInput =
      effectiveScheduleMode === 'once' || effectiveScheduleMode === 'legacy-interval';
    const runAtValue: number | null = usesDateTimeInput
      ? useNaturalSchedule && parsedNaturalSchedule.ok
        ? parsedNaturalSchedule.runAt
        : new Date(runAt).getTime()
      : computeNextScheduledRun(effectiveScheduleConfig, Date.now());
    ...
    try {
      if (editingId) {
        ...
        const nextScheduleSignature = buildScheduleSignatureFromForm(
          ...
          toLocalDateTimeInput(runAtValue),
          effectiveScheduleConfig?.times ?? selectedTimes,
          effectiveScheduleConfig?.kind === 'weekly'
            ? effectiveScheduleConfig.weekdays
            : selectedWeekdays,
          effectiveRepeatEvery ?? repeatEvery,
          effectiveRepeatUnit ?? repeatUnit
        );
        ...
        const shouldResetScheduleTime =
          ...
          (usesDateTimeInput && toLocalDateTimeInput(runAtValue) !== originalRunAtInput) ||
        ...
        const payload: ScheduleUpdateInput = {
          ...
          repeatEvery: effectiveRepeatEvery,
  ...
  function editTask(task: ScheduleTask) {
    ...
    setScheduleText(
      describeScheduleTextFromConfig({
        runAt: task.nextRunAt ?? task.runAt,
    ...
    setUseAdvancedSchedule(
      Boolean(task.repeatEvery && task.repeatUnit) || (task.scheduleConfig?.times.length ?? 0) > 1
    );
      ...
      <div className="rounded-lg border border-border bg-surface p-4 space-y-3">
        ...
        <div className="rounded-lg border border-border bg-background p-3 space-y-3">
          ...
          <div className="space-y-2">
            <div className="text-xs text-text-muted">{t('schedule.naturalLabel')}</div>
            <input
              value={scheduleText}
              onChange={(event) => {
              ...
              placeholder={t('schedule.naturalPlaceholder')}
              className="w-full px-3 py-2 rounded-lg bg-surface border border-border text-sm"
            />
            <div className="flex flex-wrap gap-2">
              {NATURAL_SCHEDULE_SUGGESTIONS.map((suggestion) => (
                <button
                  key={suggestion}
                  type="button"
                  onClick={() => {
                  ...
                  className="rounded-full border border-border px-3 py-1 text-xs text-text-secondary hover:bg-surface-hover hover:text-text-primary transition-colors"
                >
                  {suggestion}
                </button>
              ))}
            ...
            >
              {parsedNaturalSchedule.ok ? (
                <div className="space-y-1">
                  <div className="font-medium">{t('schedule.naturalParsed')}</div>
                  <div>{parsedNaturalSchedule.readableSummary}</div>
                  <div className="text-xs text-text-muted">
                    {t('schedule.nextRun', { value: formatTime(parsedNaturalSchedule.nextRunAt) })}
                  </div>
                </div>
              ) : (
                <div className="space-y-1">
                  <div>{parsedNaturalSchedule.reason}</div>
                  <div className="text-xs">
                    {t('schedule.naturalExamples', {
                      examples: joinAppList(parsedNaturalSchedule.examples),
                    })}
                  </div>
                </div>
              )}
          ...
          <button
            type="button"
            onClick={() => setUseAdvancedSchedule((current) => !current)}
            className="text-xs font-medium text-accent hover:text-accent/80 transition-colors"
          >
            {useAdvancedSchedule ? t('schedule.hideAdvanced') : t('schedule.showAdvanced')}
          </button>

          {useAdvancedSchedule && (
            <div className="rounded-lg border border-border bg-surface/70 p-3 space-y-3">
              <div
