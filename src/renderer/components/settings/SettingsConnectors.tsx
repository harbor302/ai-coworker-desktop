...
export function SettingsConnectors({
  ...
  isActive: boolean;
  presetFilter?: (key: string) => boolean;
  serverFilter?: (server: MCPServerConfig) => boolean;
  hideServers?: boolean;
  hideSummary?: boolean;
  hideCustomAdd?: boolean;
}) {
  ...
  const visibleServers = useMemo(
    () => (serverFilter ? servers.filter(serverFilter) : servers),
    [servers, serverFilter]
  );
  const filteredPresetEntries = useMemo(
    () => Object.entries(presets).filter(([key]) => !presetFilter || presetFilter(key)),
    [presets, presetFilter]
  );
  const visibleToolCount = useMemo(
    () => tools.filter((tool) => visibleServers.some((server) => server.id === tool.serverId)).length,
    [tools, visibleServers]
  );
  ...
  async function handleRefreshServer(server: MCPServerConfig) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('mcp.updateMcpFailed'));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Server List */}
      {!hideServers && !showAddForm && !editingServer && (
      ...
      {/* Preset Servers */}
      {!showAddForm && !editingServer && !configuringPreset && filteredPresetEntries.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
                  ...
                  <div
                    ...
                    className={`group relative rounded-xl border p-5 transition-all duration-200 ease-out ${
                      isAdded
                        ...
                        : 'border-border-subtle bg-surface hover:border-accent/50 hover:shadow-md hover:shadow-accent/5 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.995] cursor-pointer'
                    }`}
                    onClick={() => {
                      if (!isAdded) handleAddPreset(key);
                    }}
                  >
                    <div className="flex items-center gap-4">
                      {/* Icon */}
                      <div
                        className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center border overflow-hidden transition-all duration-200 ${
                          isAdded
                            ? 'bg-background border-border-subtle'
                            : 'bg-background border-border-subtle group-hover:border-accent/30 group-hover:shadow-[0_0_10px_rgba(99,102,241,0.1)]'
                        }`}
                      >
                        {preset.icon ? (
                          <img
                            src={
                              preset.icon.startsWith('/')
                                ? `${import.meta.env.BASE_URL}${preset.icon.slice(1)}`
                                : preset.icon
                            }
                            alt={preset.name}
                            className="w-6 h-6 object-contain connector-icon"
                          />
                        ) : (
                          <Plug
                            className={`w-5 h-5 transition-colors duration-200 ${isAdded ? 'text-text-muted' : 'text-text-muted group-hover:text-accent'}`}
                          />
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h4
                            className={`font-semibold text-sm transition-colors duration-200 ${isAdded ? 'text-text-primary' : 'text-text-primary group-hover:text-accent'}`}
                          >
                            {preset.name}
                          </h4>
                          {requiresConfig && !isAdded && (
                            <span className="px-1.5 py-px text-[10px] font-medium rounded bg-warning/10 text-warning border border-warning/20 transition-transform duration-200 group-hover:scale-105">
                              {t('mcp.requiresToken')}
                            </span>
                          )}
                        </div>
                        <p
                          className={`text-xs leading-relaxed line-clamp-2 transition-colors duration-200 ${isAdded ? 'text-text-muted' : 'text-text-muted group-hover:text-text-secondary'}`}
                        >
                          {preset.description ||
                            (preset.type === 'stdio'
                      ...
                      <div className="shrink-0 flex items-center">
                        {isAdded ? (
                          <div className="w-8 h-8 rounded-full bg-success/10 flex items-center justify-center text-success shadow-[0_0_6px_rgba(34,197,94,0.3)]">
                          ...
                          <button
                            ...
                            disabled={isLoading}
                            className="w-8 h-8 rounded-full border border-border-muted bg-background flex items-center justify-center text-text-secondary hover:text-accent hover:border-accent hover:bg-accent/5 hover:shadow-[0_0_8px_rgba(99,102,241,0.2)] active:scale-90 transition-all duration-150 disabled:opacity-50"
                            title={requiresConfig ? t('mcp.configure') : t('common.add')}
                          >
                    ...
                    {!isAdded && (
                      <div className="absolute bottom-0 left-4 right-4 h-0.5 rounded-full bg-gradient-to-r from-accent/0 via-accent/0 to-accent/0 group-hover:from-accent/0 group-hover:via-accent/60 group-hover:to-accent/0 transition-all duration-300" />
                    )}
                  </div>
                );
              })}
      ...
      {/* Add Custom Button */}
      {!hideCustomAdd && !showAddForm && !editingServer && (
        <button
      ...
      <div className="text-sm text-text-muted text-center pt-2">
        {t('mcp.toolsAvailable', { count: visibleToolCount })}
      </div>
    </div>
  );
}
...
function ServerCard({
  ...
  return (
    <div className="group rounded-xl border border-border bg-surface overflow-hidden hover:border-accent/40 hover:shadow-md hover:shadow-accent/5 transition-all duration-200 ease-out">
      <div className="p-4">
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <span className="px-2 py-0.5 text-xs rounded bg-surface-muted text-text-muted transition-transform duration-200 group-hover:scale-105">
                {server.type.toUpperCase()}
              </span>
              {status?.version && (
                <span
                  className="px-2 py-0.5 text-xs rounded bg-background/70 text-text-muted border border-border-subtle"
                  title={t('mcp.version')}
                >
                  v{status.version}
                </span>
          ...
          <div className="flex items-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity duration-200">
            ...
            <button
              ...
              className="p-2 rounded-lg bg-surface-muted text-text-secondary hover:bg-surface-active hover:text-text-primary active:scale-90 transition-all duration-150 disabled:opacity-40 disabled:cursor-not-allowed"
              title={t('mcp.updateMcp')}
            >
              <RefreshCw
                className={`w-4 h-4 ${serverStatus === 'connecting' ? 'animate-spin' : ''}`}
              />
            </button>
            <button
              onClick={onEdit}
              disabled={isLoading}
              className="p-2 rounded-lg bg-surface-muted text-text-secondary hover:bg-surface-active hover:text-text-primary active:scale-90 transition-all duration-150"
              title={t('common.edit')}
            >
            ...
            <button
              onClick={onDelete}
              disabled={isLoading}
              className="p-2 rounded-lg bg-error/10 text-error hover:bg-error/20 hover:shadow-[0_0_8px_rgba(239,68,68,0.25)] active:scale-90 transition-all duration-150"
