...
import { Send, Square, Loader2, Plug, X, Clock, Paperclip, RotateCcw } from 'lucide-react';
...
export function ChatView() {
  ...
  const handleReplay = () => {
    if (activeSessionId) {
      startReplay(activeSessionId);
    }
  };

  const handleStopReplay = () => {
    if (activeSessionId) {
      stopReplay(activeSessionId);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-background">
      ...
      <div
        ref={headerRef}
        className="titlebar-drag relative h-12 border-b border-border-muted grid grid-cols-[1fr_auto_1fr] items-center px-4 lg:px-8 bg-background/88 backdrop-blur-md"
      >
        <div className="flex items-center gap-2">
          {messages.length > 0 && !isSessionRunning && (
            <button
              type="button"
              onClick={isReplaying ? handleStopReplay : handleReplay}
              className="flex shrink-0 min-w-max items-center gap-1 whitespace-nowrap px-2 py-0.5 rounded-md text-[10px] font-medium text-text-muted hover:text-text-primary hover:bg-surface-hover transition-colors"
              title={isReplaying ? t('chat.stopReplay') : t('chat.replay')}
            >
              {isReplaying ? (
              ...
              {isReplaying ? t('chat.replaying') : t('chat.replay')}
            </button>
          )}
        </div>
        <h2
        ...
        >
          {displayedMessages.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-28 text-text-muted space-y-3 text-center">
              <p className="text-[11px] uppercase tracking-[0.16em] text-text-muted/80">cowoker</p>
      ...
      <div className="border-t border-border-muted bg-background/92 backdrop-blur-md">
        <div className="max-w-[920px] mx-auto px-5 lg:px-8 py-5">
          ...
          >
            ...
            <div
              className={`flex items-center gap-2 p-3.5 rounded-[1.75rem] bg-background/88 border border-border-muted shadow-soft transition-colors ${
            ...
            >
              ...
              <div className="relative flex-1 h-9">
                ...
                <textarea
                  ...
                  className="w-full resize-none bg-transparent border-none outline-none text-text-primary placeholder:text-text-muted text-[15px] leading-9 py-0 h-9"
                />
              </div>

              <div className="flex items-center gap-2">
                <PermissionModeSelector className="hidden sm:block" />
                <ChatModelSelector align="right" className="hidden sm:block" />
