...
export function ContextPanel() {
  ...
  return (
    <div
      className={`relative h-full transition-all duration-300 ease-out bg-background border-l border-border-muted text-sm ${contextPanelCollapsed ? 'w-10' : 'w-72'}`}
    >
      {/* Collapsed layer */}
      <div
        className={`absolute inset-0 flex items-start justify-center pt-3 transition-opacity duration-200 ${contextPanelCollapsed ? 'opacity-100' : 'opacity-0 invisible pointer-events-none'}`}
      >
        <button
      ...
      {/* Expanded layer */}
      <div
        className={`absolute inset-0 flex flex-col overflow-hidden transition-opacity duration-200 ${contextPanelCollapsed ? 'opacity-0 invisible pointer-events-none' : 'opacity-100'}`}
      >
        {/* Header */}
        <div className="px-3 h-12 flex items-center gap-2 border-b border-border-muted shrink-0">
          <button
            onClick={toggleContextPanel}
