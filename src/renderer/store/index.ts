...
import { startSessionReplay, stopSessionReplay } from './replay';

export type GlobalNoticeType = 'info' | 'warning' | 'error' | 'success';
export type GlobalNoticeAction = 'open_api_settings';
export type PluginsTab = 'dataAuthorization' | 'skills' | 'webBridge';
...
interface AppState {
  ...
  // Replay actions
  replayState: Record<string, 'idle' | 'playing'>;
  startReplay: (sessionId: string) => void;
  stopReplay: (sessionId: string) => void;

  // Session completion indicators (sessions that finished running while not active)
  unreadCompletedSessionIds: Set<string>;
  markSessionRead: (sessionId: string) => void;

  // Completion flash animation tracking
  completionFlashIds: Set<string>;
  clearCompletionFlash: (sessionId: string) => void;
...
export const useAppStore = create<AppState>((set) => ({
  ...
  updateSession: (sessionId, updates) =>
    set((state) => {
      ...
      const nextSessions = applySessionUpdate(state.sessions, sessionId, updates);
      const nextSession = nextSessions.find((s) => s.id === sessionId);

      let unreadCompletedSessionIds = state.unreadCompletedSessionIds;
      let completionFlashIds = state.completionFlashIds;
      if (
        prevSession &&
        nextSession &&
        ...
        completionFlashIds = new Set(completionFlashIds);
        completionFlashIds.add(sessionId);
        if (state.activeSessionId !== sessionId) {
          unreadCompletedSessionIds = new Set(unreadCompletedSessionIds);
          unreadCompletedSessionIds.add(sessionId);
        }
      }
  ...
  removeSession: (sessionId) =>
    set((state) => {
      ...
      const unreadCompletedSessionIds = new Set(state.unreadCompletedSessionIds);
      unreadCompletedSessionIds.delete(sessionId);
      const completionFlashIds = new Set(state.completionFlashIds);
      completionFlashIds.delete(sessionId);
      return {
  ...
  removeSessions: (sessionIds) =>
    set((state) => {
      ...
      for (const id of sessionIds) {
        unreadCompletedSessionIds.delete(id);
        completionFlashIds.delete(id);
      }
  ...
  setActiveSession: (sessionId) =>
    set((state) => {
      const updates: Partial<AppState> = { activeSessionId: sessionId };
      if (sessionId && state.unreadCompletedSessionIds.has(sessionId)) {
        updates.unreadCompletedSessionIds = new Set(state.unreadCompletedSessionIds);
        updates.unreadCompletedSessionIds.delete(sessionId);
      }
      return updates;
    }),
  ...
  markSessionRead: (sessionId) =>
    set((state) => {
      if (!state.unreadCompletedSessionIds.has(sessionId)) return {};
      const unreadCompletedSessionIds = new Set(state.unreadCompletedSessionIds);
      unreadCompletedSessionIds.delete(sessionId);
      return { unreadCompletedSessionIds };
    }),

  clearCompletionFlash: (sessionId) =>
    set((state) => {
      if (!state.completionFlashIds.has(sessionId)) return {};
      const completionFlashIds = new Set(state.completionFlashIds);
      completionFlashIds.delete(sessionId);
      return { completionFlashIds };
    }),
  ...
  startReplay: (sessionId) => {
    set((state) => ({
      replayState: { ...state.replayState, [sessionId]: 'playing' },
    }));
    startSessionReplay(sessionId).then(() => {
      set((state) => ({
        replayState: { ...state.replayState, [sessionId]: 'idle' },
      }));
    });
  },
...
if (typeof window !== 'undefined') {
  ...
  w.__navigate = (page: string, tab?: string, sessionId?: string) => {
    ...
    if (page === 'welcome') {
      ...
      store.setShowSettings(false);
      store.setShowSchedule(false);
      store.setPluginsTab('dataAuthorization');
      store.setShowPlugins(true);
      store.setActiveSession(null);
    } else if (page === 'skills') {
      store.setShowSettings(false);
      store.setShowSchedule(false);
      store.setPluginsTab('skills');
      store.setShowPlugins(true);
      store.setActiveSession(null);
    } else if (page === 'plugins') {
      store.setShowSettings(false);
      store.setShowSchedule(false);
      if (tab === 'skills' || tab === 'webBridge' || tab === 'dataAuthorization') {
        store.setPluginsTab(tab);
      }
      store.setShowPlugins(true);
      store.setActiveSession(null);
      ...
      store.setShowSchedule(true);
      store.setActiveSession(null);
    } else if (page === 'webBridge') {
      store.setShowSettings(false);
      store.setShowSchedule(false);
      store.setPluginsTab('webBridge');
      store.setShowPlugins(true);
      store.setActiveSession(null);
