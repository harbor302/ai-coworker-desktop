...
import type { ModelQuickPlan, ModelQuickPlanTemplate } from '../../shared/model-quick-plans';
import {
...
type CreateMode = 'blank' | 'clone' | 'template';
...
const DEFAULT_CONFIG_SET_NAME_ZH = '默认供应商';
...
function normalizeProfile(
  ...
  const useManualModel =
    typeof profile?.useCustomModel === 'boolean' ? profile.useCustomModel : false;
  return {
    apiKey: profile?.apiKey || '',
    baseUrl:
      profileKey === 'ollama' ? normalizeOllamaBaseUrl(rawBaseUrl) || fallback.baseUrl : rawBaseUrl,
    model: modelValue,
    customModel: useManualModel ? modelValue : '',
    useCustomModel: useManualModel,
...
function toPersistedProfiles(
  ...
  for (const key of PROFILE_KEYS) {
    ...
    persisted[key] = {
      ...
      model: finalModel,
      useCustomModel: profile.useCustomModel,
...
export function buildApiConfigDraftSignature(
  ...
  return JSON.stringify({
    ...
    profiles: PROFILE_KEYS.map((key) => ({
      ...
      useCustomModel: Boolean(persisted[key]?.useCustomModel),
    })),
  });
}
  ...
  if (config?.configSets && config.configSets.length > 0) {
    return config.configSets.map((set, index) => {
      ...
      for (const key of PROFILE_KEYS) {
        ...
        normalizedProfiles[key] = {
          ...
          model: uiProfile.useCustomModel
            ...
            : uiProfile.model,
          useCustomModel: uiProfile.useCustomModel,
        };
      }
      ...
      if (isMigratedOllamaSet) {
        ...
        normalizedProfiles.ollama = {
          ...
          model: ollamaProfile.useCustomModel
            ...
            : ollamaProfile.model,
          useCustomModel: ollamaProfile.useCustomModel,
        };
      }
      ...
      return {
        ...
        name:
          typeof set.name === 'string' && set.name.trim() ? set.name : `供应商配置 ${index + 1}`,
        provider,
        customProtocol,
        activeProfileKey,
...
export function useApiConfigState(options: UseApiConfigStateOptions = {}) {
  ...
  const hasFetchedModelOptions = Object.prototype.hasOwnProperty.call(
    discoveredModels,
    activeProfileKey
  );
  const modelOptions = discoveredModels[activeProfileKey] || [];
  ...
  const shouldShowOllamaManualModelToggle = true;
  ...
  }, [activeProfileKey, baseUrl, currentPreset.baseUrl, provider]);
  ...
  const refreshModelOptions = useCallback(async () => {
    ...
    const requestedCustomProtocol = customProtocol;
    const requestedBaseUrl =
      requestedProvider === 'custom' || requestedProvider === 'ollama'
        ? baseUrl.trim()
        : (baseUrl.trim() || currentPreset.baseUrl || '').trim();
    ...
    try {
      const models = await window.electronAPI.config.listModels({
        ...
        customProtocol: requestedCustomProtocol,
        apiKey: apiKey.trim(),
      ...
      dispatch({
        ...
        updater: (current) => {
          const currentModel =
            (current.useCustomModel ? current.customModel.trim() : current.model.trim()) ||
            current.model.trim();
          const discoveredMatch = models.find((item) => item.id === currentModel);
          const nextModel = discoveredMatch?.id || models[0]?.id || current.model;

          return {
            ...current,
            model: nextModel,
            useCustomModel: models.length > 0 ? false : current.useCustomModel,
          };
        },
      });

      if (models.length === 0) {
        showErrorKey('api.noModelsAvailable');
      } else {
        showSuccessKey('api.modelsFetched', { count: models.length });
      ...
      dispatch({
        ...
        updater: (current) => {
          ...
          const currentModel =
            ...
            current.model.trim();
          const discoveredMatch = models.find((item) => item.id === currentModel);
          const nextModel = discoveredMatch?.id || autoSelectModelId || current.model;
  ...
  const createConfigSet = useCallback(
    async (payload: { name: string; mode: CreateMode; template?: ModelQuickPlanTemplate }) => {
      if (!isElectron) {
        return false;
      }
      ...
      try {
        const result = await window.electronAPI.config.createSet({
          ...
          template: payload.mode === 'template' ? payload.template : undefined,
        });
        applyPersistedConfigToStore(result.config, presets);
        showSuccessKey(
          payload.mode === 'template' ? 'api.quickPlanCreated' : 'api.configSetCreated'
        );
        setTimeout(() => clearSuccessMessage(), 1500);
        return true;
      } catch (createError) {
  ...
  const createConfigSetFromQuickPlan = useCallback(
    async (plan: ModelQuickPlan) => {
      ...
      return createConfigSet({
        ...
        template: {
          ...
          customProtocol: plan.customProtocol,
    ...
    [createConfigSet, handleSave, hasUnsavedChanges]
  );
  ...
  const canDeleteCurrentConfigSet = Boolean(currentConfigSet && configSets.length > 1);
