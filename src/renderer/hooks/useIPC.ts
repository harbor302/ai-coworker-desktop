...
export function useIPC() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        // Hydrate main-process permission rules and mode from the renderer's
        // (possibly persisted) settings so the agent has them before the first
        // tool call. Re-read the store after applyConfigSnapshot so we send the
        // latest persisted values, not a stale pre-hydration snapshot.
        try {
          ...
          window.electronAPI.send({
            ...
            payload: {
              ...
              permissionMode: latest.settings.permissionMode,
  ...
  const changeWorkingDir = useCallback(
    async (
      sessionId?: string,
      currentPath?: string,
      options?: { title?: string; allowCreateDirectory?: boolean }
      ...
      return invoke<{ success: boolean; path: string; error?: string }>({
        type: 'workdir.select',
        payload: { sessionId, currentPath, ...options },
      });
    },
    [invoke]
  );
  ...
  const setWorkingDir = useCallback(
    async (path: string, sessionId?: string): Promise<{ success: boolean; path: string; error?: string }> => {
      if (!isElectron) {
