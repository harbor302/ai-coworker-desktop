...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  mcp: {
    ...
    testConnection: (input: McpConnectionTestInput): Promise<McpConnectionTestResult> =>
      ipcRenderer.invoke('mcp.testConnection', input),
  },

  // Cowoker Web Bridge methods
  webBridge: {
    getStatus: (): Promise<WebBridgeStatus> => ipcRenderer.invoke('webBridge.getStatus'),
    startDaemon: (): Promise<{ success: boolean; error?: string }> =>
      ipcRenderer.invoke('webBridge.startDaemon'),
    stopDaemon: (): Promise<{ success: boolean; error?: string }> =>
      ipcRenderer.invoke('webBridge.stopDaemon'),
    showExtensionFolder: (): Promise<{ success: boolean; error?: string }> =>
      ipcRenderer.invoke('webBridge.showExtensionFolder'),
  },
  ...
  skills: {
    ...
    getDetail: (skillId: string): Promise<{ success: boolean; content: string | null }> =>
      ipcRenderer.invoke('skills.getDetail', skillId),
  },
...
declare global {
  interface Window {
    electronAPI: {
      ...
      webBridge: {
        getStatus: () => Promise<WebBridgeStatus>;
        startDaemon: () => Promise<{ success: boolean; error?: string }>;
        stopDaemon: () => Promise<{ success: boolean; error?: string }>;
        showExtensionFolder: () => Promise<{ success: boolean; error?: string }>;
      };
