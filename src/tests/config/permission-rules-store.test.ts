...
import {
  decidePermission,
  disableSessionAutoAllow,
  enableSessionAutoAllow,
  forgetSessionPermissions,
  getPermissionRules,
  rememberAlwaysAllow,
  setPermissionMode,
...
describe('permission-rules-store', () => {
  ...
  describe('session-scoped auto-allow', () => {
    it('allows every tool within the enabled session', () => {
      setPermissionRules([
        { tool: 'bash', action: 'deny' },
        { tool: 'write', action: 'ask' },
      ]);

      enableSessionAutoAllow(SESSION_A);

      expect(decidePermission(SESSION_A, 'bash', { command: 'rm -rf /tmp/x' })).toBe('allow');
      expect(decidePermission(SESSION_A, 'write', { path: '/tmp/x' })).toBe('allow');
      expect(decidePermission(SESSION_A, 'unknown_tool', {})).toBe('allow');
    });

    it('does NOT leak auto-allow to other sessions', () => {
      setPermissionRules([{ tool: 'bash', action: 'deny' }]);

      enableSessionAutoAllow(SESSION_A);

      expect(decidePermission(SESSION_A, 'bash', { command: 'ls' })).toBe('allow');
      expect(decidePermission(SESSION_B, 'bash', { command: 'ls' })).toBe('deny');
    });

    it('forgetSessionPermissions clears session auto-allow', () => {
      setPermissionRules([{ tool: 'bash', action: 'ask' }]);
      enableSessionAutoAllow(SESSION_A);
    ...
    it('disableSessionAutoAllow clears only full-session auto-allow', () => {
      setPermissionRules([{ tool: 'bash', action: 'ask' }]);
      enableSessionAutoAllow(SESSION_A);

      disableSessionAutoAllow(SESSION_A);
