...
// Get cowoker data directory for persistent storage
// Use platform-appropriate paths:
// - macOS: ~/Library/Application Support/coworker
// - Windows: %APPDATA%/coworker
const COWORKER_DATA_DIR =
  PLATFORM === 'win32'
    ? path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'coworker')
    : path.join(os.homedir(), 'Library', 'Application Support', 'coworker');

// Directory for storing GUI operate files (screenshots, etc.)
const GUI_OPERATE_DIR = path.join(COWORKER_DATA_DIR, 'gui_operate');
...
const GUI_APPS_DIR = path.join(COWORKER_DATA_DIR, 'gui_apps');
...
function getResourcesDirCandidates(): string[] {
  ...
  const envResources = process.env.COWORKER_RESOURCES_PATH;
...
async function resolveCliclickPath(): Promise<string | null> {
  ...
  const envOverride = process.env.COWORKER_CLICLICK_PATH;
...
async function resolvePythonExec(): Promise<PythonExec | null> {
  ...
  // 1) Explicit override (useful for debugging)
  const envPython = process.env.COWORKER_PYTHON_PATH;
  const envPythonHome = process.env.COWORKER_PYTHON_HOME;
  ...
  try {
    ...
    if (/Accessibility privileges not enabled/i.test(result.stderr || '')) {
      const hint =
        ...
        '- System Settings → Privacy & Security → Accessibility：允许 cowoker\n' +
    ...
    const hint =
      '\n\nmacOS 权限提示 / Permissions:\n' +
      '- System Settings → Privacy & Security → Accessibility：允许 cowoker\n' +
      '- System Settings → Privacy & Security → Automation：允许 cowoker 控制 “System Events”\n';
...
async function takeScreenshot(
  ...
  try {
    ...
    const hint =
      '\n\nmacOS 权限提示 / Permissions:\n' +
      '- System Settings → Privacy & Security → Screen Recording：允许 cowoker\n' +
...
async function takeScreenshotForDisplay(
  ...
  const disableImageOutput = process.env.COWORKER_DISABLE_IMAGE_TOOL_OUTPUT === '1';
...
async function callVisionAPIWithTimeout(
  ...
  if (isOpenAICompatible) {
    ...
    if (isOpenRouter) {
      headers['X-Title'] = 'AI Coworker Desktop';
    }
