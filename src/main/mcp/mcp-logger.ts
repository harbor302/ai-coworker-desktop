...
function getLogsDirectory(): string {
  ...
  if (platform === 'darwin') {
    ...
    appDataDir = path.join(os.homedir(), 'Library', 'Application Support', 'coworker');
  } else if (platform === 'win32') {
    // Windows: %APPDATA%/coworker
    appDataDir = path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'coworker');
  } else {
    // Linux: ~/.config/coworker
    appDataDir = path.join(os.homedir(), '.config', 'coworker');
  }
