...
export function findPreferredUnixNpxPath(
  pathEnv: string | undefined,
  bundledNpxPath: string | null,
  pathExists: (candidate: string) => boolean = (candidate) => {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
  ...
  const bundledNormalized = bundledNpxPath ? path.resolve(bundledNpxPath) : null;
  ...
  for (const rawEntry of (pathEnv || '').split(':')) {
    ...
    const candidate = path.join(entry, 'npx');
    if (bundledNormalized && path.resolve(candidate) === bundledNormalized) {
      continue;
    }
    if (pathExists(candidate)) {
      return candidate;
...
export class MCPManager {
  ...
  private async resolvePreferredNpxPath(pathEnv: string | undefined): Promise<string> {
    ...
    if (process.platform === 'win32') {
      ...
      if (!preferredNpxPath) {
        throw new Error(
          'npx is not available. Install Node.js so cowoker can use your system npx.cmd, or reinstall the app to restore the bundled runtime.'
        );
      }
    ...
    const preferredNpxPath = findPreferredUnixNpxPath(pathEnv, bundledNpxPath);
    if (!preferredNpxPath) {
      throw new Error(
        'npx is not available. Install Node.js/npm so cowoker can launch npm-based MCP servers.'
      );
    }

    this.npxPath = preferredNpxPath;
    if (bundledNpxPath && path.resolve(preferredNpxPath) !== path.resolve(bundledNpxPath)) {
      log(`[MCPManager] Using system npx: ${this.npxPath}`);
    } else {
      log(`[MCPManager] Using bundled npx: ${this.npxPath}`);
    }
   ...
   */
  private getAzureImageServerPath(): string {
    return this.getMcpServerPath('azure-image-server.ts');
  }

  private getBilibiliMcpServerPath(): string {
    return this.getMcpServerPath('bilibili-server.ts');
  }

  private getTwitterMcpServerPath(): string {
    return this.getMcpServerPath('twitter-server.ts');
  }

  private getXiaohongshuMcpServerPath(): string {
    return this.getMcpServerPath('xiaohongshu-server.ts');
  }

  /**
   * Check whether a server is a built-in MCP server that is bundled
   * with the app and launched via getMcpServerPath().
   */
  private isBuiltinMcpServer(name: string): boolean {
    return (
      name === 'GUI_Operate' ||
      name === 'GUI Operate' ||
  ...
  private async connectServerInternal(config: MCPServerConfig): Promise<void> {
    ...
    if (config.type === 'stdio') {
      ...
      const isBuiltinServer = this.isBuiltinMcpServer(config.name);
      const isOldConfig =
      ...
      const isConfluenceServer =
        config.name.trim().toLowerCase() === 'confluence' ||
        config.name.trim().toLowerCase() === 'confluence mcp';
      const isXiaohongshuServer =
        config.name.trim().toLowerCase() === 'xiaohongshu' ||
        config.name.trim().toLowerCase() === 'xiaohongshu mcp' ||
        config.name.trim() === '小红书';

      if (isConfluenceServer) {
        log(`[MCPManager] Launching ${config.name} through npx package`);
        command = 'npx';
      ...
      args = args.map((arg) => {
        ...
        if (arg === '{AZURE_IMAGE_SERVER_PATH}') {
          return this.getAzureImageServerPath();
        }
        // Bilibili TypeScript MCP compiled server path
        if (arg === '{BILIBILI_MCP_SERVER_PATH}') {
          return this.getBilibiliMcpServerPath();
        }
        // Twitter TypeScript MCP compiled server path
        if (arg === '{TWITTER_MCP_SERVER_PATH}') {
          return this.getTwitterMcpServerPath();
        }
        if (arg === '{XIAOHONGSHU_MCP_SERVER_PATH}') {
          return this.getXiaohongshuMcpServerPath();
        }
        return arg;
      });
      ...
      if (isXiaohongshuServer && !configEnv.XIAOHONGSHU_STARTUP_CHECK) {
        configEnv.XIAOHONGSHU_STARTUP_CHECK = 'bridge';
      }
      const env = await this.getEnhancedEnv(configEnv);
      ...
      if (app.isPackaged && isBuiltinServer) {
        ...
        env.COWORKER_RESOURCES_PATH = process.resourcesPath || '';
      }
  ...
  getServerStatus(): Array<{
    ...
    for (const [serverId, config] of this.serverConfigs.entries()) {
      ...
      status.push({
        ...
        status: serverStatus,
        toolCount,
        version: getMcpConfigVersion(config),
      });
    }
...
function getMcpConfigVersion(config: MCPServerConfig): string | undefined {
  if (config.type !== 'stdio') {
    return undefined;
  }
  ...
  const packageSpec = (config.args || []).find((arg) => {
  ...
  return parseNpxPackageVersion(packageSpec) || 'latest';
}

function parseNpxPackageVersion(packageSpec: string): string | undefined {
  if (!packageSpec.trim()) {
    return undefined;
  }
  ...
  if (packageSpec.startsWith('@')) {
    ...
    return versionIndex === -1 ? undefined : packageSpec.slice(versionIndex + 1);
  }
