...
import type { ModelQuickPlanTemplate } from '../../shared/model-quick-plans';
...
export type CreateSetMode = 'blank' | 'clone' | 'template';

export interface CreateConfigSetPayload {
  name: string;
  mode?: CreateSetMode;
  fromSetId?: string;
  template?: Partial<ModelQuickPlanTemplate>;
}
...
export interface AppConfig {
  ...
  // Built-in web tools configuration
...
export type WebSearchEngine = 'exa' | 'tavily' | 'brave';
export type WebFetchEngine = 'exa' | 'native';

export interface WebToolsConfig {
  searchEngine: WebSearchEngine;
  fetchEngine: WebFetchEngine;
  apiKeys: {
    exa: string;
    tavily: string;
    brave: string;
...
/**
 * Model presets intentionally do not include hardcoded model lists.
 * Users fetch live models from their configured API endpoint instead.
 */
export async function getProviderPresets(): Promise<typeof PROVIDER_PRESETS> {
...
function isWebSearchEngine(value: unknown): value is WebSearchEngine {
  return value === 'exa' || value === 'tavily' || value === 'brave';
}

function isWebFetchEngine(value: unknown): value is WebFetchEngine {
  return value === 'exa' || value === 'native';
}

function normalizeWebToolsConfig(raw: unknown): WebToolsConfig {
  const value = typeof raw === 'object' && raw !== null ? (raw as Partial<WebToolsConfig>) : {};
  const rawKeys =
    typeof value.apiKeys === 'object' && value.apiKeys !== null
      ? (value.apiKeys as Partial<WebToolsConfig['apiKeys']>)
      : {};
  return {
    searchEngine: isWebSearchEngine(value.searchEngine)
      ? value.searchEngine
      : defaultConfig.webTools.searchEngine,
    fetchEngine: isWebFetchEngine(value.fetchEngine)
      ? value.fetchEngine
      : defaultConfig.webTools.fetchEngine,
    apiKeys: {
      exa: typeof rawKeys.exa === 'string' ? rawKeys.exa : defaultConfig.webTools.apiKeys.exa,
      tavily:
        typeof rawKeys.tavily === 'string' ? rawKeys.tavily : defaultConfig.webTools.apiKeys.tavily,
      brave:
        typeof rawKeys.brave === 'string' ? rawKeys.brave : defaultConfig.webTools.apiKeys.brave,
    },
  };
}
...
export class ConfigStore {
  ...
  constructor() {
    ...
    this.store = createEncryptedStoreWithKeyRotation<AppConfigRecord>({
      stableKey: 'coworker-config-stable-v1',
      legacyKeys: [
        'coworker-config-v1',
        ...getLegacyDerivedKeyHexes({
          moduleDirname: __dirname,
          stableSeed: 'coworker-config-stable-v1',
          legacySeed: 'coworker-config-v1',
          salt: 'coworker-config-salt',
        }),
      ],
  ...
  private normalizeProfile(
    ...
    if (typeof profile?.useCustomModel === 'boolean') {
      result.useCustomModel = profile.useCustomModel;
    }
  ...
  private normalizeConfig(rawConfig: Partial<AppConfig> | undefined): AppConfig {
    ...
    const result: AppConfig = {
      ...
      webTools: normalizeWebToolsConfig(raw.webTools),
  ...
  private buildTemplateConfigSet(payload: {
    id: ConfigSetId;
    name: string;
    template: Partial<ModelQuickPlanTemplate> | undefined;
  }): ApiConfigSet {
    const rawTemplate = payload.template || {};
    const provider = isProviderType(rawTemplate.provider) ? rawTemplate.provider : 'custom';
    const customProtocol = isCustomProtocol(rawTemplate.customProtocol)
      ? rawTemplate.customProtocol
      : defaultProtocolForProvider(provider);
    const activeProfileKey = profileKeyFromProvider(provider, customProtocol);
    const defaultProfile = this.getDefaultProfile(activeProfileKey);
    const baseUrl =
      typeof rawTemplate.baseUrl === 'string' ? rawTemplate.baseUrl.trim() : defaultProfile.baseUrl;
    const model =
      typeof rawTemplate.model === 'string' ? rawTemplate.model.trim() : defaultProfile.model;
    const profiles = this.cloneProfiles(undefined);
    const activeProfile = this.normalizeProfile(activeProfileKey, {
      apiKey: '',
      baseUrl: baseUrl || defaultProfile.baseUrl,
      model: model || defaultProfile.model,
      contextWindow: rawTemplate.contextWindow,
      maxTokens: rawTemplate.maxTokens,
    });

    // For blank custom templates, keep empty values so the UI opens as a true fill-in form.
    if (provider === 'custom') {
      activeProfile.baseUrl = baseUrl;
      activeProfile.model = model;
    }

    profiles[activeProfileKey] = activeProfile;
    ...
    return {
      ...
      activeProfileKey,
      profiles,
      enableThinking: Boolean(rawTemplate.enableThinking),
      updatedAt: nowISO(),
    };
  }
  ...
  createSet(payload: CreateConfigSetPayload): AppConfig {
    ...
    const mode: CreateSetMode =
      payload.mode === 'blank' || payload.mode === 'template' ? payload.mode : 'clone';
    let newSet: ApiConfigSet;

    if (mode === 'template') {
      newSet = this.buildTemplateConfigSet({
        id,
        name,
  ...
  deleteSet(payload: { id: string }): AppConfig {
    ...
    const fallbackActive = nextSets[0]?.id;
  ...
  update(updates: Partial<AppConfig>): void {
    ...
    this.saveConfig({
      ...
      webTools:
        updates.webTools !== undefined
          ? normalizeWebToolsConfig(updates.webTools)
          : current.webTools,
      isConfigured:
  ...
  applyToEnv(): void {
    ...
    // Clear web tools env vars so removed keys are not inherited from previous sessions
    delete process.env.WEBSEARCH_ENGINE;
    delete process.env.WEBFETCH_ENGINE;
    ...
    // Apply built-in web tools configuration to environment
    const webTools = projectedConfig.webTools;
    if (webTools) {
      if (isWebSearchEngine(webTools.searchEngine)) {
        process.env.WEBSEARCH_ENGINE = webTools.searchEngine;
      }
      if (isWebFetchEngine(webTools.fetchEngine)) {
        process.env.WEBFETCH_ENGINE = webTools.fetchEngine;
      }
      const exaKey = webTools.apiKeys?.exa?.trim();
      if (exaKey) {
        process.env.EXA_API_KEY = exaKey;
      }
      const tavilyKey = webTools.apiKeys?.tavily?.trim();
      if (tavilyKey) {
        process.env.TAVILY_API_KEY = tavilyKey;
      }
      const braveKey = webTools.apiKeys?.brave?.trim();
      if (braveKey) {
