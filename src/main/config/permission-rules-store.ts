...
/** Global permission mode: 'ask' shows dialog, 'auto_allow' skips all dialogs. */
let permissionMode: 'ask' | 'auto_allow' = 'ask';
...
/** Session-scoped full auto-allow, used for trusted background sessions such as scheduled tasks. */
const autoAllowSessions = new Set<string>();
...
export function setPermissionMode(mode: unknown): void {
  if (mode === 'auto_allow') {
...
export function getPermissionMode(): 'ask' | 'auto_allow' {
 ...
 *
 * Matching order:
 *   1. Global permission mode (auto_allow bypasses everything)
 *   2. Session-scoped full auto-allow
 *   3. Session-scoped "always allow" memory
 *   4. First rule whose `tool` matches (case-insensitive) AND whose
...
export function decidePermission(
  ...
  // Global auto-allow mode: skip all dialogs
  if (permissionMode === 'auto_allow') return 'allow';

  if (autoAllowSessions.has(sessionId)) return 'allow';
...
export function enableSessionAutoAllow(sessionId: string): void {
  autoAllowSessions.add(sessionId);
}

export function disableSessionAutoAllow(sessionId: string): void {
  autoAllowSessions.delete(sessionId);
}
