...
import { listProviderModels } from './config/model-listing';
import { setPermissionRules, setPermissionMode } from './config/permission-rules-store';
import { mcpConfigStore } from './mcp/mcp-config-store';
import {
  getCowokerDaemonStatus,
  startCowokerDaemon,
  stopCowokerDaemon,
  showCowokerExtensionFolder,
} from './mcp/cowoker-daemon-launcher';
...
// Hardware acceleration is enabled by default for smooth window resizing and UI animations.
// Set COWORKER_DISABLE_GPU=1 only when debugging GPU/driver compatibility issues.
if (process.env.COWORKER_DISABLE_GPU === '1') {
...
app
  ...
  .then(async () => {
    ...
    skillsManager = new SkillsManager(db, {
      ...
      getConnectorServers: () => mcpConfigStore.getServers(),
      watchStorage: true,
    });
    ...
    sessionManager.setSkillsManager(skillsManager);
    ...
    if (process.platform === 'darwin') {
      ...
      try {
        ...
        console.log('[App] Failed to set dock icon:', e);
      }
    }
  ...
  .catch((error) => {
    ...
    dialog.showErrorBox('cowoker 启动失败', `${message}\n\n请查看日志获取更多信息。`);
    app.quit();
  });
...
async function cleanupSandboxResources(): Promise<void> {
  ...
  stopNavServer();
  stopCowokerDaemon().catch((error) => {
    logError('[App] Error stopping cowoker daemon:', error);
  });
...
ipcMain.handle(
  'mcp.testConnection',
  async (_event, input: { presetKey: string; env?: Record<string, string> }) => {
    try {
      const { testMcpConnection } = await import('./mcp/mcp-connection-tests');
      return await testMcpConnection(input);
    } catch (error) {
      logError('[MCP] Error testing connector connection:', error);
      return {
        ok: false,
        presetKey: input.presetKey,
        name: input.presetKey,
        latencyMs: 0,
        summary: 'MCP connection test failed',
        details: error instanceof Error ? error.message : String(error),
      };
    }
  }
...
// Cowoker Web Bridge IPC handlers
ipcMain.handle('webBridge.getStatus', async () => {
  try {
    return await getCowokerDaemonStatus();
  } catch (error) {
    logError('[WebBridge] Error getting daemon status:', error);
    return {
      healthy: false,
      running: false,
...
ipcMain.handle('webBridge.startDaemon', async () => {
  try {
    return await startCowokerDaemon();
  } catch (error) {
    logError('[WebBridge] Error starting daemon:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    };
...
ipcMain.handle('webBridge.stopDaemon', async () => {
  try {
    return await stopCowokerDaemon();
  } catch (error) {
    logError('[WebBridge] Error stopping daemon:', error);
    return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
  }
});

ipcMain.handle('webBridge.showExtensionFolder', async () => {
  try {
    return showCowokerExtensionFolder();
  } catch (error) {
    logError('[WebBridge] Error showing extension folder:', error);
    return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
  }
});
...
ipcMain.handle('skills.getDetail', async (_event, skillId: string) => {
  try {
    if (!skillsManager) {
      throw new Error('SkillsManager not initialized');
    }
    const content = skillsManager.getSkillDetail(skillId);
    return { success: true, content };
  } catch (error) {
    logError('[Skills] Error getting skill detail:', error);
    throw error;
  }
});
...
ipcMain.handle('logs.export', async () => {
  try {
    ...
    const result = await dialog.showSaveDialog(mainWindow!, {
      title: 'Export Logs',
      defaultPath: `coworker-logs-${new Date().toISOString().split('T')[0]}.zip`,
      filters: [
...
async function handleClientEvent(event: ClientEvent): Promise<unknown> {
  ...
  switch (event.type) {
    ...
    case 'workdir.select': {
      ...
      if (event.payload.allowCreateDirectory) {
        properties.push('createDirectory');
      }
      const workdirResult = await dialog.showOpenDialog(mainWindow!, {
        properties,
        title: event.payload.title || 'Select Working Directory',
    ...
    case 'settings.update': {
      ...
      const payloadMode = (event.payload as { permissionMode?: unknown }).permissionMode;
      if (payloadMode === 'ask' || payloadMode === 'auto_allow') {
        setPermissionMode(payloadMode);
      }
      return null;
    }
