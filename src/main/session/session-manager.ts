...
import { enableSessionAutoAllow, forgetSessionPermissions } from '../config/permission-rules-store';
import {
  log,
  logError,
  logWarn,
...
import { generateTitleWithPiSdk } from '../agent/pi-one-shot';
...
export class SessionManager {
   ...
   */
  private initializeAgentRunner(): void {
    this.agentRunner = this.buildAgentRunner();
    log('[SessionManager] Using cowoker agent runner');
  }

  setSkillsManager(skillsManager: SkillsManager): void {
    this.skillsManager = skillsManager;
    if (this.agentRunner && 'setSkillsManager' in this.agentRunner) {
      (this.agentRunner as AgentRunner).setSkillsManager(skillsManager);
    }
  }
  ...
  async startSession(
    ...
    if (options?.autoAllowPermissions) {
      enableSessionAutoAllow(session.id);
    }

    // Start processing the prompt with content blocks
    this.enqueuePrompt(session, prompt, content);

    // The running status event can reach the renderer before it has inserted
    // this new session, so return a running snapshot for the initial render.
  ...
  private async generateTitleWithConfig(titlePrompt: string): Promise<string | null> {
    ...
    return normalizeGeneratedTitle(await generateTitleWithPiSdk(titlePrompt, configStore.getAll()));
  }
  ...
  private async processQueue(session: Session): Promise<void> {
    ...
    try {
      ...
      // The outer loop drains normal arrivals. If the run was explicitly
      // stopped, prompts sent during abort cleanup must start after cleanup.
      ...
      if (controller.signal.aborted && queue && queue.length > 0) {
        const latestSession = this.loadSession(session.id);
        if (latestSession) {
          log('[SessionManager] Restarting queue after stopped run:', session.id);
  ...
  // Also clears SDK session cache because Pi SDK sessions are bound to cwd
  updateSessionCwd(sessionId: string, cwd: string): void {
    ...
    // (Pi SDK sessions cannot change cwd mid-session)
