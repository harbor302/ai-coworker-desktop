...
import { MCP_CONNECTOR_REGISTRY, getConnectorDefinitionForServer } from '../mcp/connector-registry';
import type { MCPServerConfig as ConnectorMCPServerConfig } from '../mcp/mcp-manager';
...
interface SkillsManagerOptions {
  ...
  getConnectorServers?: () => ConnectorMCPServerConfig[];
 ...
 * 2. Global: <userData>/claude/skills/ (only skills installed through this app)
 * 3. Built-in skills
 */
export class SkillsManager {
  ...
  private getConnectorServersFn?: () => ConnectorMCPServerConfig[];
  ...
  constructor(db: DatabaseInstance, options: SkillsManagerOptions = {}) {
    ...
    this.getConnectorServersFn = options.getConnectorServers;
  ...
  private loadConnectorBuiltinSkills(): Skill[] {
    for (const key of Array.from(this.loadedSkills.keys())) {
      if (key.startsWith('builtin-connector-')) {
        this.loadedSkills.delete(key);
      }
    }

    const servers = this.getConnectorServersFn?.() || [];
    const skills: Skill[] = [];

    for (const definition of Object.values(MCP_CONNECTOR_REGISTRY)) {
      const skillFilePath = definition.skillPath();
      if (!skillFilePath) {
        continue;
      }

      const sourcePath = path.dirname(skillFilePath);
      const metadata = this.getSkillMetadata(sourcePath);
      if (!metadata) {
        continue;
      }

      const hasEnabledConnector = servers.some((server) => {
        const serverDefinition = getConnectorDefinitionForServer(server);
        return server.enabled && serverDefinition?.key === definition.key;
      });

      const skill: Skill = {
        id: `builtin-connector-${definition.key}`,
        name: metadata.name,
        description: metadata.description,
        type: 'builtin',
        enabled: hasEnabledConnector,
        locked: true,
        lockReason: hasEnabledConnector
          ? `${definition.displayName} connector manages this skill automatically.`
          : `Enable the ${definition.displayName} connector to use this skill.`,
        managedBy: definition.displayName,
        createdAt: Date.now(),
        sourcePath,
      };
  ...
  setSkillEnabled(skillId: string, enabled: boolean): void {
    ...
    if (skill) {
      if (skill.locked) {
        throw new Error('This skill is managed automatically and cannot be toggled here');
      }
   ...
   */
  getAllSkills(): Skill[] {
    this.loadConnectorBuiltinSkills();
   ...
   */
  getSkillDetail(skillId: string): string | null {
    const skill = this.loadedSkills.get(skillId);
    ...
    try {
      ...
      logError(`Failed to read SKILL.md for ${skillId}:`, error);
      return null;
    }
  }
