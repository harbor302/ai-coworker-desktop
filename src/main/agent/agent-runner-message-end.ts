import type { AssistantMessage, TextContent, ThinkingContent, ToolCall } from './pi-sdk';
...
export function shouldAutoRetryError(errorText: string): boolean {
...
export function toUserFacingErrorText(errorText: string): string {
  ...
  if (lower.includes('cannot continue from message role')) {
    return `会话状态异常：AI 上下文历史出现不连续（最后一条消息为 assistant 且无待处理指令）。已自动重置会话缓存，请重新发送消息即可恢复。`;
  }
  return errorText;
}
...
export function resolveMessageEndPayload(
  ...
  if (message?.stopReason === 'error' && message.errorMessage) {
    return {
      ...
      shouldAutoRetry: shouldAutoRetryError(message.errorMessage),
      nextStreamedText,
