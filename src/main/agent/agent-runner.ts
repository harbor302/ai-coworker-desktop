 ...
 *
 * Responsibilities:
 * - Runs AI conversations via the cowoker agent SDK (createAgentSession)
...
import { createPiAgentSession, createPiCodingToolDefinitions } from './pi-runtime';
...
import { connectorProfileStore } from '../mcp/connector-profile-store';
import { buildConnectorProfileTools } from '../mcp/connector-profile-tools';
import {
  getConnectorDefinitionForServer,
  getRelatedSkillPathsForMcpServers,
} from '../mcp/connector-registry';
import {
  log,
  logWarn,
  logError,
...
function extractPromptImages(content: ContentBlock[] | undefined): PiImageContent[] {
  if (!content) return [];
 ...
 */
function buildMcpUsageInstructionsPrompt(servers: MCPServerConfig[]): string {
  const connectorInstructions = servers
  ...
  const uniqueInstructions = Array.from(
    new Map(
      connectorInstructions.map((item) => [`${item.connectorName}:${item.skillPath}`, item])
    ).values()
  );

  const connectorLines = uniqueInstructions.map(
    (item) => `- ${item.connectorName} MCP (${item.serverName}): ${item.skillPath}`
  );

  return `<mcp_usage_instructions>
Before directly calling MCP tools for a user task, consult the relevant MCP usage instructions first.
- For any connector listed below, read its SKILL.md with the Read tool before the first domain tool call in the session, unless you have already read it in this session.
- Follow that skill's workflow, especially initialization/profile checks, safe read-only exploration, and result formatting.
- If the connector profile status says "missing" or "stale", do not immediately explore; first explain briefly and ask the user whether to run the skill's read-only profiling flow.
- If no connector-specific skill is listed for an MCP server, inspect the tool descriptions and use the least-invasive read/list/status tools before mutation.

Connector usage files:
${connectorLines.length > 0 ? connectorLines.join('\n') : '- none'}
</mcp_usage_instructions>`;
}

function getMcpToolUsagePrefix(
  mcpTool: ReturnType<MCPManager['getTools']>[number],
  serverConfigs: MCPServerConfig[]
): string {
  const server = serverConfigs.find((config) => config.id === mcpTool.serverId);
  const definition = server ? getConnectorDefinitionForServer(server) : null;
  const skillPath = definition?.skillPath();

  if (definition && skillPath) {
    return [
      `MCP usage rule: Before using ${definition.displayName} MCP tools for a user task, read and follow the connector usage skill at ${skillPath} if you have not already read it in this session.`,
      'Check connector_profile_status before domain calls; if the profile is missing or stale, ask the user whether to run the read-only profiling flow described in the skill.',
    ].join(' ');
  }

  return 'MCP usage rule: Before using this MCP server for a task, consult any available MCP/connector instructions in the system prompt and prefer read/list/status calls before mutations.';
}
...
function buildMcpCustomTools(
  ...
  return mcpTools.map((mcpTool) => {
    ...
    const toolDef: ToolDefinition<TSchema, unknown> = {
      ...
      description: [
        getMcpToolUsagePrefix(mcpTool, serverConfigs),
          ...
          return {
            ...
            details:
              normalizedResult.images.length > 0
                ? { coworkerImages: normalizedResult.images }
                : undefined,
          };
        } catch (err: unknown) {
...
/**
 * AgentRunner - Uses the Pi agent SDK
 *
...
export class AgentRunner {
  ...
  invalidateSkillsSetup(): void {
    ...
    // Existing SDK sessions keep the system prompt that was built at creation
    // time. Dispose them so toggled-off skills disappear on the next turn.
...
${hints.join('\n')}
  ...
  private async resolveSkillPaths(
    ...
    // When SkillsManager is available, return only enabled skills' SKILL.md paths
    // instead of entire directories (which would load ALL skills regardless of enabled state).
    if (this._skillsManager) {
      const activeSkills = await this._skillsManager.getActiveSkills(sessionId || '', projectPath);
      for (const skill of activeSkills) {
        if (skill.sourcePath) {
          const skillMdPath = path.join(skill.sourcePath, 'SKILL.md');
          if (fs.existsSync(skillMdPath)) {
            mergedPaths.add(skillMdPath);
          }
        }
      }
    } else {
      ...
      for (const p of basePaths) {
        if (p && fs.existsSync(p)) mergedPaths.add(p);
      }
    }
    ...
    if (this._pluginRuntimeService) {
      try {
        ...
        logWarn('[AgentRunner] Failed to resolve runtime plugin skill paths:', error);
      }
    }
    ...
    for (const connectorSkillPath of getRelatedSkillPathsForMcpServers(enabledMcpServers)) {
      if (fs.existsSync(connectorSkillPath)) {
        mergedPaths.add(connectorSkillPath);
      }
    }
  ...
  private getBuiltinSkillsPath(): string {
    ...
    for (const p of possiblePaths) {
      if (fs.existsSync(p)) {
        log('[AgentRunner] Found built-in skills at:', p);
        return p;
      }
    }
  ...
  constructor(
    ...
    log('[AgentRunner] Initialized with cowoker agent SDK');
   ...
   * `agent.beforeToolCall`. This is the only interception point that
  ...
  private installPermissionHook(piSession: PiAgentSession, sessionId: string): void {
    ...
    if (!agent || (typeof agent.setBeforeToolCall !== 'function' && !('beforeToolCall' in agent))) {
      logWarn('[AgentRunner] Cannot access agent.beforeToolCall — skipping permission hook');
      return;
    }
    ...
    const sdkBeforeToolCall: ((ctx: any, signal?: AbortSignal) => Promise<any>) | undefined =
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (agent as any)._beforeToolCall || (agent as any).beforeToolCall;

    const requestPermission = this.requestPermission;
    const getDisplayName = (name: string): string => this.getToolDisplayName(name);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const beforeToolCall = async (ctx: any, signal?: AbortSignal): Promise<any> => {
    ...
    if (typeof agent.setBeforeToolCall === 'function') {
      agent.setBeforeToolCall(beforeToolCall);
    } else {
      agent.beforeToolCall = beforeToolCall;
    }

    log(`[AgentRunner] Permission hook installed on session ${sessionId} via agent.beforeToolCall`);
  }
      ...
      return {
        ...
        execute: async (
          ...
          if (AgentRunner.isSudoCommand(command)) {
            ...
            log('[AgentRunner] Executing sudo command with password injection (via stdin pipe)');
            try {
    ...
    try {
      ...
      if (sandbox.isWSL && sandbox.wslStatus?.distro && workingDir) {
        ...
        if (syncResult.success) {
          ...
          log(`[AgentRunner] Sandbox initialized: ${sandboxPath}`);
          log(
          ...
          log('[AgentRunner] Falling back to /mnt/ access (less secure)');
      ...
      if (sandbox.isLima && sandbox.limaStatus?.instanceRunning && workingDir) {
        ...
        if (syncResult.success) {
          ...
          logError('[AgentRunner] Sandbox sync failed:', syncResult.error);
          log('[AgentRunner] Falling back to direct access (less secure)');
      ...
      if (hasImages) {
        log('[AgentRunner] User message contains images:', promptImages.length);
      }
      ...
      if (apiKey) {
        ...
        log('[AgentRunner] Set runtime API key for config provider:', piProvider);
      } else {
        if (provider === 'ollama') {
          log(
      ...
      // Use app-specific agent config directory to avoid conflicts with user settings.
      // Keep the legacy on-disk folder name `claude` so existing installed skills continue to work.
      ...
      if (!this._skillsSetupDone) {
        ...
        // Ensure app agent config directory exists
        if (!fs.existsSync(userAgentDir)) {
          fs.mkdirSync(userAgentDir, { recursive: true });
        }

        // Ensure app agent skills directory exists
        const appSkillsDir = this.getRuntimeSkillsDir();
        if (!fs.existsSync(appSkillsDir)) {
          fs.mkdirSync(appSkillsDir, { recursive: true });
        }

        // The runtime skills directory is reserved for skills explicitly installed
        // through this app. Built-ins are loaded directly from their bundled path.
        this.syncConfiguredSkillsToRuntimeDir(appSkillsDir);
      }
      ...
      log('[AgentRunner] App agent dir:', userAgentDir);
      ...
      logCtx('[AgentRunner] Enable thinking mode:', enableThinking);
      ...
      const connectorProfilePrompt = connectorProfileStore.buildPromptContextForServers(allConfigs);
      const mcpUsageInstructionsPrompt = buildMcpUsageInstructionsPrompt(allConfigs);
      ...
      const skillsSignature = JSON.stringify({
        skillPaths,
        connectorProfilePrompt,
        mcpUsageInstructionsPrompt,
      });
      ...
      if (cachedSession && cachedSession.runtimeSignature !== sessionRuntimeSignature) {
        logCtx('[AgentRunner] Runtime changed, recreating cached pi session:', session.id);
        try {
          cachedSession.session.dispose();
        } catch (disposeError) {
          logWarn('[AgentRunner] dispose error while recreating pi session:', disposeError);
        }
        this.piSessions.delete(session.id);
        cachedSession = undefined;
      }
      if (cachedSession && cachedSession.skillsSignature !== skillsSignature) {
        logCtx('[AgentRunner] Skills changed, recreating cached pi session:', session.id);
        try {
          cachedSession.session.dispose();
        } catch (disposeError) {
          logWarn(
      ...
      if (this.mcpManager) {
          ...
          for (const config of allConfigs) {
            try {
              ...
              if (config.type === 'stdio') {
                ...
                if (!imageCapable) {
                  serverEnv.COWORKER_DISABLE_IMAGE_TOOL_OUTPUT = '1';
                }
                ...
                const hasPlaceholders = resolvedArgs.some(
                  (arg) =>
                    ...
                    arg.includes('{AZURE_IMAGE_SERVER_PATH}') ||
                    arg.includes('{BILIBILI_MCP_SERVER_PATH}') ||
                    arg.includes('{TWITTER_MCP_SERVER_PATH}') ||
                    arg.includes('{XIAOHONGSHU_MCP_SERVER_PATH}')
                );
                ...
                if (hasPlaceholders) {
                  ...
                  } else if (config.name === 'Azure Image') {
                    presetKey = 'azure-image';
                  } else if (config.name === 'Bilibili') {
                    presetKey = 'bilibili';
                ...
                // 当命令是 npx 或 node 时优先使用内置路径，并注入内置 node bin。
                if (bundledNodePaths && (command === 'npx' || command === 'node')) {
                ...
                if (command === 'npx' && bundledNpx) {
                  command = bundledNpx;
                } else if (command === 'node' && bundledNodePaths) {
                  command = bundledNodePaths.node;
                }

                const normalizedServerName = config.name.trim().toLowerCase();
                if (
                  (normalizedServerName === 'xiaohongshu' ||
                    normalizedServerName === 'xiaohongshu mcp' ||
                    normalizedServerName === '小红书') &&
                  !serverEnv.XIAOHONGSHU_STARTUP_CHECK
                ) {
                  serverEnv.XIAOHONGSHU_STARTUP_CHECK = 'bridge';
                }
        ...
        log('[AgentRunner] Final mcpServers summary:', safeStringify(mcpServersSummary, 2));
...
This is an isolated sandbox environment. Use ${VIRTUAL_WORKSPACE_PATH} as the root path for file operations.
      ...
      const coworkAppendPrompt = [
        'You are an cowoker assistant. Be concise, accurate, and tool-capable.',
...
6. ALL OUTPUTS GO TO WORKSPACE: When creating files, generating reports, saving data, or writing any output, always save them to the current workspace directory. Use relative paths or the workspace root path. Only save to a different location if the user explicitly requests it.`,
        workspaceInfoPrompt,
...
- Before using MCP tools for a domain connector such as Confluence, first consult the MCP usage instructions/skill listed in <mcp_usage_instructions>. Do not jump straight to domain MCP calls when connector instructions exist.
</tool_behavior>`,
        mcpUsageInstructionsPrompt,
        connectorProfilePrompt,
      ...
      const mcpCustomTools = this.mcpManager
        ? buildMcpCustomTools(this.mcpManager, allConfigs)
        : [];
      const extensionCustomTools = extensionResult.customTools || [];
      const connectorProfileTools = buildConnectorProfileTools(allConfigs);
      const customTools = [...mcpCustomTools, ...connectorProfileTools, ...extensionCustomTools];
      ...
      if (connectorProfileTools.length > 0) {
        log(
          `[AgentRunner] Registered ${connectorProfileTools.length} connector profile tools:`,
          connectorProfileTools.map((t) => t.name).join(', ')
        );
      }
      ...
      const codingTools = createPiCodingToolDefinitions(
        effectiveCwd,
        bashOptions ? { bash: bashOptions } : undefined
      );
      const webTools = createWebTools();
      const builtinTools = [...codingTools, ...webTools];

      // Inject a default 120s timeout for bash commands when the model omits one
      const withTimeout = AgentRunner.wrapBashToolWithDefaultTimeout(builtinTools);
      ...
      log(
        `[AgentRunner] Built-in tools (${wrappedTools.length}): ${wrappedTools.map((t: { name?: string; type?: string }) => t.name || t.type).join(', ')}`
      );
      log(
        `[AgentRunner] Custom tools (${customTools.length}): ${customTools.map((t) => t.name).join(', ')}`
      );
      ...
      if (cachedSession) {
        ...
        logCtx('[AgentRunner] Reusing cached pi session for:', session.id);
        ...
        const { session: newPiSession } = await createPiAgentSession({
          model: piModel,
          thinkingLevel,
          authStorage,
          tools: wrappedTools,
      ...
      const unsubscribe = piSession.subscribe((event) => {
        try {
          ...
          switch (event.type) {
            case 'message_update': {
              ...
              if (ame.type === 'text_delta') {
                ...
                logCtxError('[AgentRunner] pi-ai stream error:', ame.reason, errorDetail);
              }
              break;
            }
            ...
            case 'message_end': {
              ...
              if (process.env.COWORK_LOG_SDK_MESSAGES_FULL === '1') {
                log('[AgentRunner] message_end raw message:', safeStringify(msg, 2));
              }
                    ...
                    content: [
                      {
                        ...
                        text: `**Error**: ${resolvedPayload.errorText}\n\n${
                          resolvedPayload.shouldAutoRetry
                            ? '_Agent 正在自动重试，请稍候..._'
                            : '_请检查配置后重试。_'
                        }`,
                      },
            ...
            case 'compaction_start': {
              log('[AgentRunner] Compaction started, reason:', event.reason);
            ...
            case 'compaction_end': {
              ...
              log('[AgentRunner] Compaction ended:', title, 'willRetry:', event.willRetry);
              if (compactionStepId) {
          ...
          logError('[AgentRunner] Error in subscribe callback:', subscribeErr);
          if (compactionStepId) {
      ...
      if (controller.signal.aborted && abortedByLoopGuard) {
        logCtx('[AgentRunner] Aborted by loop guard (detected after prompt returned)');
        return;
      }
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        if (abortedByTimeout) {
          ...
          logCtx('[AgentRunner] Aborted by loop guard');
        } else {
        ...
        // Defensive: pi SDK session state corruption — last message is assistant
        // with no queued steering/follow-up messages. Clear cache so next turn
        // becomes a cold start and rebuilds history from DB.
        if (rawErrorText.includes('Cannot continue from message role: assistant')) {
          logWarn(
            '[AgentRunner] Clearing pi session cache due to assistant-role continuation error'
          );
