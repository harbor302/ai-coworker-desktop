import { getModel, type Api, type Model } from './pi-sdk';
...
export function applyPiModelRuntimeOverrides(
  ...
  // DeepSeek V4 models on custom/relay endpoints need replayed thinking
  // preserved as provider-specific assistant reasoning metadata.
  if (nextModel.api === 'openai-completions' && DEEPSEEK_V4_MODEL_PATTERN.test(nextModel.id)) {
    const currentCompat = (nextModel.compat || {}) as Record<string, unknown>;
    if (!currentCompat.requiresReasoningContentOnAssistantMessages) {
      nextModel = {
        ...nextModel,
        compat: {
          ...currentCompat,
