  ...
  try {
    ...
    for (const legacyKey of legacyKeys) {
      try {
        ...
        // Move the legacy file out of the way before opening with the new key.
        // Conf/electron-store reads during construction and would otherwise
        // throw on the still-encrypted legacy payload before we can rewrite it.
        ...
        if (backupPath) {
          options.log?.(`${options.logPrefix} Migrated encrypted store to a stable key`, {
            storePath,
            backupPath,
          });
        }
