...
const SRC_MCP_CONNECTORS_DIR = path.join(SRC_MCP_DIR, 'connectors');
...
const servers = [
  ...
  },
  {
    ...
    entry: 'azure-image-server.ts',
    description: 'Azure Image MCP Server',
  },
  {
    name: 'bilibili-server',
    entry: 'bilibili-server.ts',
    description: 'Bilibili MCP Server',
  },
  {
    name: 'twitter-server',
    entry: 'twitter-server.ts',
    description: 'Twitter MCP Server',
  },
  {
    name: 'xiaohongshu-server',
    entry: 'xiaohongshu-server.ts',
...
async function stageConnectorResources(stagedDir = STAGED_MCP_DIR) {
  if (!fs.existsSync(SRC_MCP_CONNECTORS_DIR)) {
    return;
  }

  const destinationDir = path.join(stagedDir, 'connectors');
  await copyDirectoryContentsWithRetries(SRC_MCP_CONNECTORS_DIR, destinationDir);
  console.log(
    `[bundle:mcp] Staged MCP connector resources at ${path.relative(PROJECT_ROOT, destinationDir)}`
  );
}
