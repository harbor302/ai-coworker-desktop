...
/**
 * Prepare a bundled Python runtime for Coworker (macOS/Linux).
 *
 * Goal:
...
const PYTHON_MINOR = process.env.COWORKER_PYTHON_MINOR || '3.10';
const ABI = `cp${PYTHON_MINOR.replace('.', '')}`; // e.g. 3.12 -> cp312

const GITHUB_REPO = process.env.COWORKER_PYTHON_STANDALONE_REPO || 'astral-sh/python-build-standalone';
...
const TARGETS = {
  darwin: {
    arm64: {
      ...
      envUrlKey: 'COWORKER_PYTHON_STANDALONE_URL_DARWIN_ARM64',
    },
    x64: {
      triple: 'x86_64-apple-darwin',
      platformTag: 'macosx_11_0_x86_64',
      envUrlKey: 'COWORKER_PYTHON_STANDALONE_URL_DARWIN_X64',
    },
  },
  linux: {
    x64: {
      ...
      envUrlKey: 'COWORKER_PYTHON_STANDALONE_URL_LINUX_X64',
    },
  },
};
...
function download(url, dest) {
  return new Promise((resolve, reject) => {
    ...
    const request = https.get(
      ...
      {
        headers: {
          'User-Agent': 'coworker-build-script',
          Accept: '*/*',
        },
      },
      (response) => {
...
function installPackages(siteDir, platformTag, pythonBin) {
  ...
  const pipPython = process.env.COWORKER_PIP_PYTHON || pythonBin;
